...
describe('claude review agent service', () => {
  ...
  it('returns no-op when no pending lead is claimable and does not consult stored sessions', async () => {
    const context = await setupApiContext()
    const claudeReview = await import('./claude-review-agent')
    ...
    const noWork = await claudeReview.claimNextPredictionLeadForClaudeReviewAgent({
      agentKey: 'review-default',
      trigger: 'scheduled',
      promptSummary: 'Check whether any pending lead is available.',
      workspaceCwd: '/tmp/review-default',
      leaseSeconds: 900,
    })
  ...
  it('completes runs by storing a review result and clearing the lease without mutating lead status', async () => {
    const context = await setupApiContext()
    const claudeReview = await import('./claude-review-agent')
